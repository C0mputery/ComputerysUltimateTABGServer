...
namespace ComputerysUltimateTABGServer.Packets
{
    public static partial class PacketTypes
    {
        ...
        {
            if (room.TryToGetPlayer(receivedPacketRaw[0], out Player? player) && room.m_Groups.TryGetValue(player.m_GroupIndex, out Group? group)) //TODO: We need a ne`w method to check if a peer is the player they say they are since **we can't trust the client**
            {
