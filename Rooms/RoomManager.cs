...
namespace ComputerysUltimateTABGServer.Rooms
{
    public static class RoomManager
    {
        ...
        private static void UpdateRoomTick(Room room)
        {
        ...
        {
            while (room.m_EnetServer.CheckEvents(out room.m_EnetEvent) > 0) { RoomPacketSwitch(room); }
            if (room.m_EnetServer.Service(15, out room.m_EnetEvent) > 0) { RoomPacketSwitch(room); }
        }
        private static void RoomPacketSwitch(Room room)
        {
